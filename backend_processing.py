...
# Build triage prompt
# ----------------------------
def _build_triage_prompt(symptoms_text: str, report_summary: str = "", past_history: Optional[List[str]] = None, location: str = "") -> str:
    past = ", ".join(past_history or [])
    prompt = f"""
...
# ----------------------------
def llm_predict_assessment(symptoms_text: str, report_summary: str = "", past_history: Optional[List[str]] = None, location: str = "") -> Dict[str, Any]:
    if not LLM.is_available():
        return {"error": "no_gemini", "message": "Gemini backend not initialized (check GOOGLE_API_KEY and langchain_google_genai)."}
    prompt = _build_triage_prompt(symptoms_text, report_summary, past_history or [], location)
